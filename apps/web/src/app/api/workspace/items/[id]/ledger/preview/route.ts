...
export async function POST(
  ...
  try {
    ...
    const result = await previewLedgerConfig(
      auth.user.id,
      id,
      candidateFrom(body)
    );
    return NextResponse.json({
      item: result.item,
      buckets: result.resolution.scope.bucketCounts,
      baselineCount: result.resolution.scope.baselineCount,
      predicate: result.predicate,
      bucketLists: result.resolution.contributions,
      template: result.resolution.methods[0]?.evidenceTemplate,
    });
  } catch (error) {
    ...
    console.error("[workspace] failed to preview ledger", error);
    return NextResponse.json(
      { error: "Could not preview ledger" },
      { status: 500 }
    );
  }
}
